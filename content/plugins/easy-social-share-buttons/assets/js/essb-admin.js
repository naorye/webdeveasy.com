function essb_option_activate(id) {
	...
	for (var i=1;i<29;i++) {
		var selector = "#essb-menu-"+id+"-"+i;
		if (jQuery(selector).length) {
			jQuery(selector).addClass("active-sub");
		}
	}
	
	jQuery("#essb-options-header").scrollintoview({
	      duration: "slow",
	      direction: "y"});
}

function essb_submenu_execute(id) {
	if (jQuery("#essb-submenu-"+id).length) {
		jQuery("#essb-submenu-"+id).scrollintoview({
		      duration: "slow",
		      direction: "y"});
	}
